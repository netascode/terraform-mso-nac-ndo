locals {
  fabric_connectivity = {
    ...
    sites = var.manage_site_connectivity ? [for site in try(local.ndo.sites, []) : {
      id                         = var.manage_sites && local.ndo_platform_version != "4.1" && local.ndo_platform_version != "4.2" && local.ndo_platform_version != "4.3" ? mso_site.site[site.name].id : data.mso_site.site[site.name].id
      apicSiteId                 = site.id
      platform                   = "on-premise"
...
data "mso_site" "site" {
  for_each = toset([for site in try(local.ndo.sites, []) : site.name if(!var.manage_sites || local.ndo_platform_version == "4.1" || local.ndo_platform_version == "4.2" || local.ndo_platform_version == "4.3") && var.manage_site_connectivity])
  name     = each.value
}
