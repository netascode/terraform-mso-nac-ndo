...
resource "mso_schema_template_deploy" "template" {
  for_each      = { for template in local.deploy_templates : template.key => template if var.deploy_templates && substr(try(local.ndo.version, local.ndo.defaults.ndo.version), 0, 3) < 3.7 }
