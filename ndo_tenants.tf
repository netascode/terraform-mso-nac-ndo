...
data "mso_user" "tenant_user" {
  for_each = local.ndo_platform_version != "4.2" && local.ndo_platform_version != "4.3" ? toset(local.tenant_users) : []
  username = each.value
}

data "mso_site" "tenant_site" {
  for_each = !var.manage_sites || local.ndo_platform_version == "4.1" || local.ndo_platform_version == "4.2" || local.ndo_platform_version == "4.3" || local.ndo_platform_version == "4.3" ? toset(local.tenant_sites) : []
  name     = each.value
}
...
resource "mso_tenant" "tenant" {
  ...
  dynamic "user_associations" {
    for_each = { for user in distinct(concat(try(each.value.users, []), local.default_users)) : user.name => user if local.ndo_platform_version != "4.2" && local.ndo_platform_version != "4.3" }
    content {
  ...
  dynamic "site_associations" {
    ...
    content {
      site_id               = var.manage_sites && local.ndo_platform_version != "4.1" && local.ndo_platform_version != "4.2" && local.ndo_platform_version != "4.3" ? mso_site.site[site_associations.value.name].id : data.mso_site.tenant_site[site_associations.value.name].id
