...
data "mso_site" "tenant_templates_site" {
  for_each = toset(distinct([for site in local.tenant_templates_sites : site.site_name if(!var.manage_sites || local.nd_managed) && var.manage_tenant_templates]))
  name     = each.value
}
...
locals {
  tenant_policies = flatten([
    ...
    sites = [for site in try(template.sites, []) : var.manage_sites && !local.nd_managed ? mso_site.site[site].id : data.mso_site.tenant_templates_site[site].id] }]
  ])
}
      ...
      for cluster in try(template.cluster, []) : [
        for site_name in try(template.sites, []) : [
          for site in [try([for s in try(cluster.sites, []) : s if s.name == site_name][0], {})] : {
            ...
            site_name              = site_name
            site_id                = var.manage_sites && !local.nd_managed ? mso_site.site[site_name].id : data.mso_site.tenant_templates_site[site_name].id
