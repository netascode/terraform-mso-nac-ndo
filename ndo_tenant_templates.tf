...
data "mso_site" "tenant_templates_site" {
  for_each = toset(distinct([for site in local.tenant_templates_sites : site.site_name if(!var.manage_sites || contains(["4.1", "4.2", "4.3"], local.ndo_platform_version)) && var.manage_tenant_templates]))
  name     = each.value
}
...
locals {
  tenant_policies = flatten([
    ...
    sites = [for site in try(template.sites, []) : var.manage_sites && !contains(["4.1", "4.2", "4.3"], local.ndo_platform_version) ? mso_site.site[site].id : data.mso_site.tenant_templates_site[site].id] }]
  ])
}
