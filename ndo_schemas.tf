...
data "mso_site" "template_site" {
  for_each = toset(distinct([for site in local.template_sites : site.site_name if(!var.manage_sites || contains(["4.1", "4.2", "4.3"], local.ndo_platform_version)) && var.manage_schemas]))
  name     = each.value
}
...
resource "mso_schema_site" "schema_site" {
  ...
  site_id             = var.manage_sites && !contains(["4.1", "4.2", "4.3"], local.ndo_platform_version) ? mso_site.site[each.value.site_name].id : data.mso_site.template_site[each.value.site_name].id
      ...
      for template in try(schema.templates, []) : [
        for contract in try(template.contracts, []) : [
          for site in try(template.sites, []) : {
            ...
            patch = [{
              op   = "add"
              path = "/sites/${var.manage_sites && !contains(["4.1", "4.2", "4.3"], local.ndo_platform_version) ? mso_site.site[site].id : data.mso_site.template_site[site].id}-${template.name}/contracts/-"
              value = {
                contractRef = {
      ...
      for template in try(schema.templates, []) : [
        for contract in try(template.contracts, []) : [
          for site_name in try(template.sites, []) : {
            ...
            schema_id                   = mso_schema.schema[schema.name].id
            site_id                     = var.manage_sites && !contains(["4.1", "4.2", "4.3"], local.ndo_platform_version) ? mso_site.site[site_name].id : data.mso_site.template_site[site_name].id
            template_name               = template.name
            contract_name               = "${contract.name}${local.defaults.ndo.schemas.templates.contracts.name_suffix}"
            service_graph_name          = "${contract.service_graph.name}${local.defaults.ndo.schemas.templates.service_graphs.name_suffix}"
            service_graph_schema_id     = contains(local.managed_schemas, try(contract.service_graph.schema, schema.name)) ? mso_schema.schema[try(contract.service_graph.schema, schema.name)].id : local.schema_ids[contract.service_graph.schema].id
              ...
              for site_name in try(template.sites, []) : [
                for node in try(contract.service_graph.nodes, []) : [
                  for site in try(node.provider.sites, []) : {
                    ...
                    path = "/sites/${var.manage_sites && !contains(["4.1", "4.2", "4.3"], local.ndo_platform_version) ? mso_site.site[site_name].id : data.mso_site.template_site[site_name].id}-${template.name}/contracts/${contract.name}${local.defaults.ndo.schemas.templates.contracts.name_suffix}/serviceGraphRelationship/serviceNodesRelationship/0/providerConnector/redirectPolicy"
      ...
      for template in try(schema.templates, []) : [
        for vrf in try(template.vrfs, []) : [
          for site in try(vrf.sites, []) : {
            ...
            schema_id     = mso_schema.schema[schema.name].id
            site_id       = var.manage_sites && !contains(["4.1", "4.2", "4.3"], local.ndo_platform_version) ? mso_site.site[site.name].id : data.mso_site.template_site[site.name].id
            template_name = template.name
      ...
      for template in try(schema.templates, []) : [
        for ap in try(template.application_profiles, []) : [
          for site in distinct(flatten([for epg in try(ap.endpoint_groups, []) : [for site in try(epg.sites, []) : site.name]])) : {
            ...
            schema_id     = mso_schema.schema[schema.name].id
            template_name = template.name
            site_id       = var.manage_sites && !contains(["4.1", "4.2", "4.3"], local.ndo_platform_version) ? mso_site.site[site].id : data.mso_site.template_site[site].id
